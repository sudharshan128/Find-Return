...
const TrustScoreHistory: React.FC<TrustScoreHistoryProps> = ({
  userId,
  limit = 20,
  className = '',
}) => {
  ...
  const fetchTrustLogs = async () => {
    try {
      ...
      let response;
      if (userId) {
        // Admin viewing another user's logs
      ...
      if (response.data.success) {
        ...
        setError('Failed to load trust score history');
      }
    } catch (err: any) {
      console.error('Error fetching trust logs:', err);
      setError(err.response?.data?.message || 'Failed to load trust score history');
    } finally {
      setLoading(false);
    }
  };
