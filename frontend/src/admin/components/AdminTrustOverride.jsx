...
interface AdminTrustOverrideProps {
  userId: string;
  userName: string;
  currentScore: number;
  currentLevel: string;
...
const AdminTrustOverride: React.FC<AdminTrustOverrideProps> = ({
  userId,
  userName,
  currentScore,
  currentLevel,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Failed to override trust score');
      }
    } catch (err: any) {
      console.error('Error overriding trust score:', err);
      setError(err.response?.data?.message || 'Failed to override trust score');
    } finally {
      setLoading(false);
    }
  };

  const predictedLevel = (score: number): string => {
